...
public class AccountPopupController extends GridPane implements Initializable, IUIController {
    ...
    public AccountBalance getAccountBalance() throws NullPointerException, NumberFormatException {
        ...
        if (amount == Long.MAX_VALUE) {
            this.setupErrorPopup("An account can a maximum balance of " + Long.MAX_VALUE/100);
            return null;
        } else if (amount == Long.MIN_VALUE) {
            this.setupErrorPopup("An account can a minimum balance of " + Long.MIN_VALUE/100);
            return null;
        }
